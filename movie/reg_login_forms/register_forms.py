...
import re
import datetime
from movie.models import BookUser


def check_phone(phone):
    phone_re = re.match(r'^1[3456789]\d{9}$', phone)  # \d 表示0-9
    if phone_re:
        return True
    else:
...
def check_birthday(birthday):
    ...
    if ch_birth and str(birthday) > '1910-01-01' and str(birthday) < str(
            datetime.datetime.today().strftime('%Y-%m-%d')):
        return True
    else:
        return ValidationError('生日不合法')


def check_pwd(pwd):
    ch_pwd = re.match(r'(?=.*[A-Za-z0-9\D._])(?=.*[A-Z])(?=.*[a-z]).{6,20}', pwd)  # \D 0-9的符号
    if not ch_pwd:
...
class Reg_Form(forms.Form):
    username = forms.CharField(label='用户名', required=True, max_length=20, widget=forms.TextInput(attrs={
        'class': 'form-control', 'placeholder': '请输入用户名'}), error_messages={
        'required': '必需的',
        'max_length': '用户名太长'
    })
    ...
    birehday = forms.DateField(label='生日', required=True, validators=[check_birthday],
    ...
    def clean_username(self):
        ...
        if user == '666':
            raise ValidationError('此用户不可以注册')
        return user
